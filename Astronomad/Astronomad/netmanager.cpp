...
// Static Variables
NetManager* NetManager::netManagerSingleton = 0;
...
bool NetManager::Initialise(EEntityType _eType, Game* _gameInstance)
{
...
void NetManager::StartThreads()
{
	packetData = 0;
	packetData = new char[MAX_MESSAGE_LENGTH];
	strcpy_s(packetData, strlen("") + 1, "");
	gameIsEnd = 0;

	if (currentType == SERVER)  //if network entity is a server
	{
		server = static_cast<Server*>(networkEntity);
		server->SetNetManager(netManagerSingleton); //Set the client's netmanager as this
		serverReceiveThread = std::thread(&Server::ReceiveData, server, std::ref(packetData), std::ref(gameIsEnd));
		processThread = std::thread(&Server::ProcessData, server);
		//Run a keep alive thread
		keepAliveThread = std::thread(&Server::KeepAlive, server, std::ref(gameIsEnd));
	}
	else if (currentType == CLIENT)  //if network entity is a client
	{
		client = static_cast<Client*>(networkEntity);
		client->SetNetManager(netManagerSingleton); //Set the client's netmanager as this
		clientReceiveThread = std::thread(&Client::ReceiveData, client, std::ref(packetData), std::ref(gameIsEnd));
	}
}

void NetManager::Update()
{
	if (currentType == CLIENT)  //if network entity is a client
	{
		if (client != nullptr)
		{
			...
			if (!client->GetWorkQueue()->empty())
			{
				...
				std::string temp;
				client->GetWorkQueue()->pop(temp);
				client->ProcessData(const_cast<char*>(temp.c_str()));
			}
		}
	}
}

void NetManager::StartUp()
{
...
{
	if (netManagerSingleton == 0)
	{
		netManagerSingleton = new NetManager();
	}

	return (*netManagerSingleton);
}

void NetManager::DestroyInstance()
{
	delete netManagerSingleton;
	netManagerSingleton = 0;
}

NetworkEntity* NetManager::GetNetworkEntity()
{
	return networkEntity;
}

EEntityType NetManager::GetCurrentType() const
{
	return currentType;
}


bool NetManager::ClientServersFound() const
{
	if (client != nullptr)
	{
		if (client->GetVecServerAddress()->empty() == false)
		{
			return true;
		}
	}
...
int NetManager::GetClientVecServersSize() const
{
	return (client->GetVecServerAddress()->size());
}

std::vector<std::string>* NetManager::GetClientVecServerName()
{
	return client->GetVecServerName();
}

void NetManager::ConnectClientToChosenServer(int _serverIndex)
{
	client->ConnectToChosenServer(_serverIndex);
}

std::string NetManager::GetUsername()
{
	return (gameInstance->GetUsername());
}

std::vector<std::string>* NetManager::GetVecClientName()
{
	return (server->GetVecClientName());
}

void NetManager::SendQuitCommand()
{
	Packet packet;
	packet.Serialize(COMMAND, "!q"); //Quit command

	if (currentType == CLIENT)
	{
		client->SendData(packet.packetData);
	}
	else if (currentType == SERVER)
	{
		server->SendDataAll(packet.packetData);
	}
}

void NetManager::Disconnect()
{
	gameInstance->GetCurrentLevel()->Disconnect();
}

void NetManager::ResetLevel()
{
	gameInstance->GetCurrentLevel()->ResetLevel();
}

void NetManager::PassGameUpdate(const char* _gameUpdateStr)
{
	//Sends the data to game instance's current-level for processing
	gameInstance->GetCurrentLevel()->ProcessNetworkData(_gameUpdateStr); 
}

void NetManager::SendGameUpdate(const char * _gameUpdate)
{
	if (currentType == CLIENT)
	{
		client->SendGameUpdate(_gameUpdate);
	}
	else if (currentType == SERVER)
	{
		server->SendGameUpdate(_gameUpdate);
	}
}

void NetManager::SetEntityID(int _ID)
{
	entityID = _ID;
	gameInstance->GetCurrentLevel()->SetEntityID(_ID);
}

int NetManager::GetEntityID() const
{
	return entityID;
}

char* NetManager::GetIPAddress()
{
	char *str = IPAddress;
	return str;
}

clock_t NetManager::GetClockStartTime() const
{
	return (gameInstance->GetCurrentLevel()->GetClock()->GetStartTime());
}

void NetManager::SetClockStartTime(clock_t time)
{
	gameInstance->GetCurrentLevel()->GetClock()->SetStartTime(time);
}
...
bool NetManager::GetIsInit()
{
	return isInit;
}

void NetManager::SetIsInit(bool _isInit)
{
	isInit = _isInit;
}

void NetManager::SetGameIsEnd(int isEnd)
{
	gameIsEnd = isEnd;
}
