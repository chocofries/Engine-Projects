...
Texter::Texter(std::string _text, std::string _font, glm::vec3 pos,
	...
	bool _isUI, bool _isBillboard, glm::vec3 color, float scale)
{
	text = _text;
	SetColor(color);
	SetScale(scale);
	...
	if (!isBillboard) //If not billboarding
	{
		program = _shdloader->CreateProgram("Assets/TextVert.vs", "Assets/TextFrag.fs", "TextVert0", "TextFrag0");
	}
	else //If billboarding
	{
		program = _shdloader->CreateProgram("Assets/TextVertBillboard.vs", "Assets/TextFrag.fs", "TextVertBillboard0", "TextFrag0");
	}
	...
	FT_Set_Pixel_Sizes(face, 0, 128);		//Set size to load glyph as
...
void Texter::SetIsBillboard(bool _isBillboard)
{
	isBillboard = _isBillboard;
}
